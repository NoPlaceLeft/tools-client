...
export class GenMapperGraph {
  ...
  loadHTMLContent() {
    document.getElementById('left-menu').innerHTML = '<div id="template-logo">' +
      ...
      '<button onclick="genmapper.introSwitchVisibility()" class="hint--rounded hint--right" aria-label="' + i18next.t('menu.helpAbout') + '"><img src="assets/icons/266-question.svg"></button>' +
      '<div class="dropdown" id="lang-selector">' +
      '<button aria-label="Language"><img src="assets/icons/203-earth.svg"></button>' +
  ...
  updateRadioField(field, element) {
    // add class to the element which the field inherits from
    element.attr('class', function (d) {
      const fieldValue = GenMapperGraph.getFieldValueForRadioType(field, d)
      if (fieldValue.class) {
